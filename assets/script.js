...
function showView(id, push = true) {
  ...
  if (switching && target === currentView) return;
  switching = true;
  currentView = target;
  views.forEach(v => {
    const active = v.id === `view-${target}`;
    v.classList.toggle('active', active);
    v.classList.toggle('entering', active);
  });
  ...
  setTimeout(() => {
    views.forEach(v => v.classList.remove('entering'));
    switching = false;
  }, 620);
}
...
function draw(){
  ...
  if(Math.random() < 0.010) spawnSaber();
  if(Math.random() < 0.0032 && duelBursts.length < 3) spawnDuel();
